...
// GC_TODO:    PxeUndiTable - add argument and description to function comment
{
  return MakePxeCall (
          SimpleNetworkDevice,
          PxeUndiTable,
...
// GC_TODO:    SimpleNetworkDevice - add argument and description to function comment
// GC_TODO:    PxeUndiTable - add argument and description to function comment
// GC_TODO:    rx_filter - add argument and description to function comment
...
// GC_TODO:    EFI_SUCCESS - add return value to function comment
{
  ...
  open.OpenFlag     = 0;
  open.PktFilter    = rx_filter;
  open.McastBuffer  = PxeUndiTable->R_Mcast_Buf;
...
/*++
  ...
  the parameter structure must be set to one of the values represented by the PXENV_STATUS_xxx
  constants.
  Description: This call activates the adapters network connection and sets the adapter ready to accept packets
...
/*++
  ...
  adapters individual address registers.
  typedef struct {
    PXENV_STATUS Status;
...
/*++
  ...
  the parameter structure must be set to one of the values represented by the PXENV_STATUS_xxx
  constants.
  Description: This call resets the adapters receive unit to accept a new filter, different from the one provided with
  the open call.
  typedef struct {
    PXENV_STATUS Status;
    UINT8 filter;
...
/*++
  ...
  network adapter driver usually queues the packet and calls the applications callback receive
...
/*++
  ...
  When the Network Interface HW generates an interrupt the protocol drivers interrupt service
