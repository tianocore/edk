...
typedef struct {
  ...
  EFI_LIST_ENTRY   NameValueListHead; // List of NAME_VALUE_NODE
...
typedef struct {
  ...
  EFI_LIST_ENTRY    OpCodeListHead;  // OpCodes consist of this expression (EXPRESSION_OPCODE)
} FORM_EXPRESSION;
...
typedef struct {
  ...
  EFI_LIST_ENTRY        DefaultListHead;     // nested EFI_IFR_DEFAULT list (QUESTION_DEFAULT), provide default values
  EFI_LIST_ENTRY        OptionListHead;      // nested EFI_IFR_ONE_OF_OPTION list (QUESTION_OPTION)

  EFI_IMAGE_ID          ImageId;             // nested EFI_IFR_IMAGE
  UINT8                 RefreshInterval;     // nested EFI_IFR_REFRESH, refresh interval(in seconds) for Question value, 0 means no refresh
  BOOLEAN               InSubtitle;          // nesting inside of EFI_IFR_SUBTITLE

  EFI_LIST_ENTRY        InconsistentListHead;// nested inconsistent expression list (FORM_EXPRESSION)
  EFI_LIST_ENTRY        NoSubmitListHead;    // nested nosubmit expression list (FORM_EXPRESSION)
...
typedef struct {
  ...
  EFI_LIST_ENTRY    ExpressionListHead;   // List of Expressions (FORM_EXPRESSION)
  EFI_LIST_ENTRY    StatementListHead;    // List of Statements and Questions (FORM_BROWSER_STATEMENT)
} FORM_BROWSER_FORM;
...
typedef struct {
  ...
  EFI_LIST_ENTRY                  StorageListHead;      // Storage list (FORMSET_STORAGE)
  EFI_LIST_ENTRY                  DefaultStoreListHead; // DefaultStore list (FORMSET_DEFAULTSTORE)
