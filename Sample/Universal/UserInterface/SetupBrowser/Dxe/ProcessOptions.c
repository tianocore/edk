  ...
  )
{
  ...
  SizeRequired = MenuOption->ThisTag->StorageStart + 
  ...
  //
  for (Index = MenuOption->TagIndex; (MenuOption->Tags[Index].Operand != EFI_IFR_END_FORM_OP) && 
                                     (MenuOption->Tags[Index].Operand != EFI_IFR_END_ONE_OF_OP); Index++) {
    ...
    switch (MenuOption->Tags[Index].Operand) {
      ...
      case EFI_IFR_PASSWORD_OP:
        EfiCopyMem(&NvRamMap[MenuOption->Tags[Index].StorageStart],&MenuOption->Tags[Index].Value, MenuOption->Tags[Index].StorageWidth);
        break;
          ...
          if (MenuOption->Tags[Index].Operand == EFI_IFR_ONE_OF_OPTION_OP) {
            if (OrderedList) {
              ...
              if (!Initialized) {
                ...
                for (Value = (UINT16)(MenuOption->TagIndex + 1); MenuOption->Tags[Value].Operand != EFI_IFR_END_ONE_OF_OP; Value++) {
  ...
  )
{
  ...
  for (; CurrIndex > PrevIndex; CurrIndex--) {
    ...
    for (; SearchIndex > PrevSearchIndex; PrevSearchIndex++) {
      if ((StringPtr[PrevSearchIndex] == CHAR_NULL)     || 
          (StringPtr[PrevSearchIndex] == CHAR_LINEFEED)) {
  ...
  )
{
  ...
  STATIC UINT16             CurrentVariableBackup; 
  ...
  switch (OpCode) {
    ...
    case EFI_IFR_VARSTORE_SELECT_PAIR_OP:
      ...
      EfiCopyMem(&TargetTag->VariableNumber2, &((EFI_IFR_VARSTORE_SELECT_PAIR *)FormData)->SecondaryVarId, sizeof (UINT16));
      CurrentVariableBackup = CurrentVariable;
    ...
    case EFI_IFR_EQ_ID_ID_OP:
      ...
      CurrentVariable = CurrentVariableBackup;
      break;
    ...
    case EFI_IFR_EQ_ID_LIST_OP:
      ...
      for (TempValue = 0; TempValue < TargetTag->Id2; TempValue++) {
        EfiCopyMem(&TargetTag->IntList[TempValue], &((EFI_IFR_EQ_ID_LIST *)FormData)->ValueList[TempValue], sizeof (UINT16));
      }
      TargetTag->VariableNumber = CurrentVariable;
      break;
