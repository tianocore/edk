...
typedef struct {
  ...
  UINT16  StorageStart; // These are the three values that are created to determine where in the variable the data is stored.  This should, in general, 
  ...
  UINT8   StorageWidth; // These are the three values that are created to determine where in the variable the data is stored.  This should, in general, 
                        // be allocated by the build tool.  The one major issue is, once storage is allocated for something, it can't be reallocated or we will get a mess.
  UINT16  Value;        // These are the three values that are created to determine where in the variable the data is stored.  This should, in general, 
                        // be allocated by the build tool.  The one major issue is, once storage is allocated for something, it can't be reallocated or we will get a mess.
  UINT8   Flags;    // (Default or current)
  UINT16  Key;
  UINT16  OldValue;     // Used to preserve a value during late consistency checking
  UINT16  Minimum;  
  UINT16  Maximum; 
  UINT16  Step;
  UINT16  Default;
  ...
  struct StringPart *StringList; // The string is obtained from the string list and formatted into lines and the lines are held in this linked list.  
  ...
  UINT16  VariableNumber2; // Used to define which variable the StorageStart will be pertinent for (0-based)  This is used for boolean check of ID versus ID
