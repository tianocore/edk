...
--*/
{
  ...
  do {
    ...
    //
    gST->ConOut->QueryMode (gST->ConOut, gST->ConOut->Mode->Mode, &gScreenDimensions.RightColumn, &gScreenDimensions.BottomRow);
    ...
    if (ScreenDimensions != NULL) {
      ...
      if ((gScreenDimensions.RightColumn < ScreenDimensions->RightColumn) || 
        ...
        if ((ScreenDimensions->RightColumn > ScreenDimensions->LeftColumn)       &&
            ...
            ((ScreenDimensions->BottomRow - ScreenDimensions->TopRow) > STATUS_BAR_HEIGHT + SCROLL_ARROW_HEIGHT*2 + FRONT_PAGE_HEADER_HEIGHT + FOOTER_HEIGHT + 1)) {
    ...
    if (CurrentVariable == VariableDefinition->VariableId) {
      ...
      if (VariableDefinition->NvRamMap != NULL) {
        ...
        if (Tag->StorageWidth == (UINT16)2) {
          Index = (UINT16)(VariableDefinition->NvRamMap[Tag->StorageStart] + (VariableDefinition->NvRamMap[Tag->StorageStart + 1] * 0x100));
  ...
  )
{
  ...
  for (Index = 0; RawFormSet[Index] != EFI_IFR_END_FORM_OP; ) {
    ...
    //
    if (RawFormSet[Index] == EFI_IFR_DATE_OP ||
      RawFormSet[Index] == EFI_IFR_TIME_OP) {
  ...
  )
{
  ...
  for (CurrTag = 0; RawFormSet[Index] != EFI_IFR_END_FORM_SET_OP; CurrTag++) {
    ...
    //
    TagLength = RawFormSet[Index+1];   // get the length
    ...
    switch (RawFormSet[Index]) {
      ...
      case EFI_IFR_FORM_OP:
        ...
        if (FileFormTags->VariableDefinitions == NULL) {
          ...
          IfrToFormTag (RawFormSet[Index], &FormTags->Tags[CurrTag], (VOID *)&RawFormSet[Index], FileFormTags->VariableDefinitions);
        } else {
      ...
      case EFI_IFR_VARSTORE_SELECT_PAIR_OP:
        IfrToFormTag (RawFormSet[Index], &FormTags->Tags[CurrTag], (VOID *)&RawFormSet[Index], NULL);
        EfiCopyMem(&CurrentVariable2, &((EFI_IFR_VARSTORE_SELECT_PAIR *)&RawFormSet[Index])->SecondaryVarId, sizeof (UINT16));
        break;
      ...
      case EFI_IFR_ORDERED_LIST_OP:
        ...
        //
        QuestionIndex = (UINT16)CurrTag;   // record for setting default later
        break;

      case EFI_IFR_ONE_OF_OPTION_OP:
        IfrToFormTag (RawFormSet[Index], &FormTags->Tags[CurrTag], (VOID *)&RawFormSet[Index], NULL);
        FormTags->Tags[QuestionIndex].Flags = ((EFI_IFR_ONE_OF_OPTION *)&RawFormSet[Index])->Flags;
        EfiCopyMem(&FormTags->Tags[QuestionIndex].Key, &((EFI_IFR_ONE_OF_OPTION *)&RawFormSet[Index])->Key, sizeof (UINT16));
      ...
      case EFI_IFR_DATE_OP:
        ...
        //
        GetNumericHeader (&FormTags->Tags[CurrTag], RawFormSet, (UINT16)(Index + TagLength), (UINT16)0, FileFormTags, CurrentVariable);

        //
        // The current language selected + the Date operand
        //
        FormTags->Tags[CurrTag + 1].Operand = RawFormSet[Index];
        GetNumericHeader (&FormTags->Tags[CurrTag + 1], RawFormSet, (UINT16)(Index + TagLength + RawFormSet[Index + TagLength + 1]), (UINT16)0, FileFormTags, CurrentVariable);
      ...
      case EFI_IFR_FORM_SET_OP:
        EfiCopyMem(&FormTags->Tags[CurrTag].GuidValue, &((EFI_IFR_FORM_SET *)&RawFormSet[Index])->Guid, sizeof (EFI_GUID));
        EfiCopyMem(&FormTags->Tags[CurrTag].CallbackHandle, &((EFI_IFR_FORM_SET *)&RawFormSet[Index])->CallbackHandle, sizeof (EFI_PHYSICAL_ADDRESS));
        EfiCopyMem(&FormTags->Tags[CurrTag].Class, &((EFI_IFR_FORM_SET *)&RawFormSet[Index])->Class, sizeof (UINT8));
        EfiCopyMem(&FormTags->Tags[CurrTag].SubClass, &((EFI_IFR_FORM_SET *)&RawFormSet[Index])->SubClass, sizeof (UINT8));
        EfiCopyMem(&FormTags->Tags[CurrTag].NvDataSize, &((EFI_IFR_FORM_SET *)&RawFormSet[Index])->NvDataSize, sizeof (UINT16));
      ...
      case EFI_IFR_BANNER_OP:
        if (gClassOfVfr == EFI_FRONT_PAGE_SUBCLASS) {
          ...
          EfiCopyMem (
                    &BannerData->Banner[((EFI_IFR_BANNER *)&RawFormSet[Index])->LineNumber][((EFI_IFR_BANNER *)&RawFormSet[Index])->Alignment], 
      ...
      case EFI_IFR_INCONSISTENT_IF_OP:
        EfiCopyMem(&FormTags->Tags[CurrTag].Text, &((EFI_IFR_INCONSISTENT *)&RawFormSet[Index])->Popup, sizeof (UINT16));
        gConsistencyId++;
      ...
      case EFI_IFR_EQ_ID_VAL_OP:
        ...
        EfiCopyMem(&InconsistentTags->QuestionId1, &((EFI_IFR_EQ_ID_VAL *)&RawFormSet[Index])->QuestionId, sizeof (UINT16));
      ...
      case EFI_IFR_EQ_VAR_VAL_OP:
        ...
        EfiCopyMem(&InconsistentTags->QuestionId1, &((EFI_IFR_EQ_VAR_VAL *)&RawFormSet[Index])->VariableId, sizeof (UINT16));
      ...
      case EFI_IFR_EQ_ID_ID_OP:
        ...
        EfiCopyMem(&InconsistentTags->QuestionId1, &((EFI_IFR_EQ_ID_ID *)&RawFormSet[Index])->QuestionId1, sizeof (UINT16));
        EfiCopyMem(&InconsistentTags->QuestionId2, &((EFI_IFR_EQ_ID_ID *)&RawFormSet[Index])->QuestionId2, sizeof (UINT16));
      ...
      case EFI_IFR_EQ_ID_LIST_OP:
        ...
        EfiCopyMem(&InconsistentTags->QuestionId1, &((EFI_IFR_EQ_ID_LIST *)&RawFormSet[Index])->QuestionId, sizeof (UINT16));
        EfiCopyMem(&InconsistentTags->ListLength, &((EFI_IFR_EQ_ID_LIST *)&RawFormSet[Index])->ListLength, sizeof (UINT16));
...
//  PrintStringAt ((gScreenDimensions.RightColumn - GetStringWidth(HomePageString)/2)/2, 1, HomePageString);
  PrintStringAt ((gScreenDimensions.RightColumn + gScreenDimensions.LeftColumn - GetStringWidth(HomePageString)/2)/2, 1, HomePageString);
  PrintAt (gScreenDimensions.LeftColumn + 2, gScreenDimensions.BottomRow-3, L"%c%c%s", ARROW_UP, ARROW_DOWN, gMoveHighlight);
  PrintAt (gScreenDimensions.RightColumn - (GetStringWidth(HomeEscapeString)/2)-2, gScreenDimensions.BottomRow-3, L" %s", HomeEscapeString);
  gST->ConOut->SetAttribute (gST->ConOut, EFI_TEXT_ATTR(EFI_LIGHTGRAY, EFI_BLACK));
      ...
      for (; VariableDefinition != NULL; VariableDefinition = VariableDefinition->Next) {
        ...
        if (EFI_ERROR(Status)) {
          ...
          if (Status == EFI_BUFFER_TOO_SMALL) {
            ...
            VariableDefinition->FakeNvRamMap = EfiLibAllocateZeroPool (SizeOfNvStore + VariableDefinition->VariableFakeSize);
            ASSERT(VariableDefinition->NvRamMap);
            ASSERT(VariableDefinition->FakeNvRamMap);
          ...
          if (Status == EFI_NOT_FOUND) {
            if (!EfiCompareMem(VariableDefinition->VariableName, L"Setup", 10)) {
              Status = Hii->GetDefaultImage(Hii, Handle[HandleIndex], 0, (UINT16 *)&VariableDefinition->VariableSize, VariableDefinition->NvRamMap);
              if (Status == EFI_BUFFER_TOO_SMALL) {
                VariableDefinition->NvRamMap = EfiLibAllocateZeroPool (VariableDefinition->VariableSize);
                ASSERT (VariableDefinition->NvRamMap != NULL);
                SizeOfNvStore = VariableDefinition->VariableSize;
              }
              Hii->GetDefaultImage(Hii, Handle[HandleIndex], 0, (UINT16 *)&VariableDefinition->VariableSize, VariableDefinition->NvRamMap);
            }
            Status = EFI_SUCCESS;
          }
        }
...
--*/
{
  ...
  if (Packet == NULL) {
    ...
    //
    EfiCopyMem(Buffer, Packet->IfrData, Packet->IfrData->Header.Length);
  ...
  for (Index = 0; RawFormBinary[Index] != EFI_IFR_END_FORM_SET_OP; ) {
    ...
    if (FormOp->Header.OpCode == EFI_IFR_FORM_SET_OP) {
      ...
      for (Index2 = 0; Index2 < sizeof (gFunctionKeySettingTable) / sizeof (FUNCTIION_KEY_SETTING); Index2 ++) {
        if (EfiCompareGuid(&FormOp->Guid, &(gFunctionKeySettingTable[Index2].FormSetGuid))) {
          //
