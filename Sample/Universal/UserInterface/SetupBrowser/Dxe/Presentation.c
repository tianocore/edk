  ...
  )
{
  ...
  do {
    ...
    //
    for (; (String[Index] != NARROW_CHAR) && (String[Index] != WIDE_CHAR) && (String[Index] != 0); Index++, Count = Count + IncrementValue);
...
//    ClearLines(0, LocalScreen.RightColumn, 0, BANNER_HEIGHT-1, BANNER_TEXT | BANNER_BACKGROUND);
    ClearLines(LocalScreen.LeftColumn, LocalScreen.RightColumn, LocalScreen.TopRow, FRONT_PAGE_HEADER_HEIGHT-1+LocalScreen.TopRow, BANNER_TEXT | BANNER_BACKGROUND);
//    for (Line = 0; Line < BANNER_HEIGHT; Line++) {
    for (Line = (UINT8)LocalScreen.TopRow; Line < BANNER_HEIGHT + (UINT8)LocalScreen.TopRow; Line++) {
//      for (Alignment = 0; Alignment < BANNER_COLUMNS; Alignment++) {
      for (Alignment = (UINT8)LocalScreen.LeftColumn; Alignment < BANNER_COLUMNS + (UINT8)LocalScreen.LeftColumn; Alignment++) {
        if (BannerData->Banner[Line-(UINT8)LocalScreen.TopRow][Alignment-(UINT8)LocalScreen.LeftColumn] != 0x0000) {
          StrFrontPageBanner = GetToken(BannerData->Banner[Line-(UINT8)LocalScreen.TopRow][Alignment-(UINT8)LocalScreen.LeftColumn], FrontPageHandle);
        } else {
          continue;
        }
        ...
        switch (Alignment-LocalScreen.LeftColumn) {
          ...
          case 1:
            ...
            //
            PrintStringAt (LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn)/3, Line, StrFrontPageBanner);
            break;
          case 2:
            //
            // Handle right column
            //
            PrintStringAt (LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn)*2/3, Line, StrFrontPageBanner);
            break;
        }
  ...
  if (gClassOfVfr != EFI_FRONT_PAGE_SUBCLASS) {
    ...
    for( Row = LocalScreen.TopRow + 1;                                 \
         Row <= LocalScreen.TopRow + NONE_FRONT_PAGE_HEADER_HEIGHT - 2; \
         Row++) {
  ...
  )
{
  ...
  if (gClassOfVfr != EFI_FRONT_PAGE_SUBCLASS) {
    ...
    PrintStringAt ((LocalScreen.RightColumn + LocalScreen.LeftColumn- GetStringWidth(StringPtr)/2)/2, LocalScreen.TopRow + 1, StringPtr);
  }
  ...
  if (gClassOfVfr == EFI_SETUP_APPLICATION_SUBCLASS) {
    ...
    PrintStringAt (LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn)/3, LocalScreen.BottomRow - 4, gFunctionNineString);
    PrintStringAt (LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn)*2/3, LocalScreen.BottomRow - 4, gFunctionTenString);
    PrintAt (LocalScreen.LeftColumn + 2, LocalScreen.BottomRow - 3, L"%c%c%s", ARROW_UP, ARROW_DOWN, gMoveHighlight);
    PrintStringAt (LocalScreen.LeftColumn + (LocalScreen.RightColumn - LocalScreen.LeftColumn)/3, LocalScreen.BottomRow - 3, gEscapeString);
  }
    ...
    do {
      switch (FormTags.Tags[Index].Operand) {
        ...
        case EFI_IFR_GRAYOUT_IF_OP:
          ...
          //
          for (; FormTags.Tags[Index].Operand == EFI_IFR_AND_OP || 
                 FormTags.Tags[Index].Operand == EFI_IFR_OR_OP ||
                 FormTags.Tags[Index].Operand == EFI_IFR_NOT_OP; Index++);
          ...
          // 
          if (SuppressIf) {
            Suppress = ValueIsNotValid(FALSE, FormTags.Tags[Index].ConsistencyId, &FormTags.Tags[Index], FileFormTags, &String);
            SuppressIf = FALSE;
          } else {
            GrayOut = ValueIsNotValid(FALSE, FormTags.Tags[Index].ConsistencyId, &FormTags.Tags[Index], FileFormTags, &String);
          }

          //
          // Advance to the end of the expression (Will land us at a grayoutif/suppressif or the op-code being affected)
          //
          for (; FormTags.Tags[Index].Operand == EFI_IFR_EQ_ID_VAL_OP || 
                 ...
                 FormTags.Tags[Index].Operand == EFI_IFR_OR_OP ||
                 FormTags.Tags[Index].Operand == EFI_IFR_LABEL_OP; Index++);
          break;
...
GetOut:
    do {
      ...
      if ((FormTags.Tags[Index].NumberOfLines > 0 || 
          FormTags.Tags[Index].Operand == EFI_IFR_DATE_OP ||
          FormTags.Tags[Index].Operand == EFI_IFR_TIME_OP) && !Suppress) {
  ...
  switch (Selection->ThisTag->Operand) {
    ...
    case  EFI_IFR_DATE_OP:
      ...
      if (!Selected) {
        ...
        if ((Selection->ThisTag->Operand == EFI_IFR_DATE_OP) ||
            (Selection->ThisTag->Operand == EFI_IFR_TIME_OP)) {
          PrintAt (StartColumnOfHelp, BottomRowOfHelp, L"%c%c%c%c%s", ARROW_UP, ARROW_DOWN, ARROW_RIGHT, ARROW_LEFT, gMoveHighlight);
        ...
        //
        if ((Selection->ThisTag->Operand == EFI_IFR_NUMERIC_OP) && 
            (Selection->ThisTag->Step == 0)) {
    ...
    case  EFI_IFR_STRING_OP:
      ...
      if (!Selected) {
        ...
        PrintStringAt ((LocalScreen.RightColumn - GetStringWidth(gEnterCommitString)/2)/2, BottomRowOfHelp, gEnterCommitString);
        PrintStringAt (ThdCol, BottomRowOfHelp, gEscapeString);
      }
      break;
  }
  ...
  )
{
  ...
  for (CurrTag = 0; FormData[Index] != EFI_IFR_END_FORM_SET_OP; CurrTag++) {
    ...
    //
    TagLength = FormData[Index+1];   // get the length
    ...
    switch (FormData[Index]) {
      ...
      case EFI_IFR_DATE_OP:
        ...
        //
        GetNumericHeader (&FormTags->Tags[CurrTag], FormData, (UINT16)(Index + TagLength), (UINT16)0, FileFormTags, CurrentVariable);

        //
        // The current language selected + the Date operand
        //
        FormTags->Tags[CurrTag + 1].Operand = FormData[Index];
        GetNumericHeader (&FormTags->Tags[CurrTag + 1], FormData, (UINT16)(Index + TagLength + FormData[Index + TagLength + 1]), (UINT16)0, FileFormTags, CurrentVariable);
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    //
    EfiCopyMem(&VariableDefinition->NvRamMap[Selection->ThisTag->StorageStart], gPreviousValue, Selection->ThisTag->StorageWidth);
  ...
  )
{
  ...
  while (Selection->Tags != NULL) {
    if (Selection->Previous) {
      ...
      //
      if (Selection->ThisTag->Operand == EFI_IFR_REF_OP &&
          Selection->ThisTag->Id != 0x0000 ) {
        if (Selection->ThisTag->Flags & EFI_IFR_FLAG_INTERACTIVE) {
          ExtractDynamicFormHandle(Selection, CallbackData, FileFormTagsHead, Selection->ThisTag->Id, &FormHandle, &TitleToken, &FormTags);
          goto DisplayPage;
        } else {
      ...
      if ((Selection->ThisTag->Flags & EFI_IFR_FLAG_INTERACTIVE) &&
        ...
        ExtractDynamicFormHandle(Selection, CallbackData, FileFormTagsHead, Selection->FormId, &FormHandle, &TitleToken, &FormTags);
      } else {
        ExtractFormHandle(Selection, FileFormTagsHead, Selection->FormId, &FormHandle, &TitleToken, &FormTags);
      }
    }
