  ...
  do {
    ...
    switch (Key.UnicodeChar) {
      ...
      default:
        ...
        } else if ((GetStringWidth(StringPtr) < MenuOption->ThisTag->Maximum) &&
    ...
    do {
      ...
      switch (Key.UnicodeChar) {
        ...
        default:
          ...
          gST->ConOut->SetCursorPosition (gST->ConOut, (DimensionsWidth - GetStringWidth(StringPtr)/2)/2 + gScreenDimensions.LeftColumn, Top + 3);
...
TheKey2:
    switch (Key.UnicodeChar) {
      case '+':
      case '-':
        if ((Tag->Operand == EFI_IFR_DATE_OP) ||
            (Tag->Operand == EFI_IFR_TIME_OP)) {
          Key.UnicodeChar = CHAR_NULL;
          if (Key.UnicodeChar == '+') {
          ...
          case SCAN_RIGHT:
            ...
            if (!ManualInput) {
              ...
              *Value = (UINT16)Number;
              ...
              if (Number == 4 && (NumericType == TIME_NUMERIC) && 
  ...
  )
{
  ...
  for (Index = MenuOption->TagIndex; MenuOption->Tags[Index].Operand != EFI_IFR_END_ONE_OF_OP; Index++) {
    if (MenuOption->Tags[Index].Operand == EFI_IFR_ONE_OF_OPTION_OP) {
      ...
      //
      if (Initialized) {
        for (ValueBackup = (UINT8)MenuOption->TagIndex; MenuOption->Tags[ValueBackup].Operand != EFI_IFR_END_ONE_OF_OP; ValueBackup++) {
    ...
    switch (Key.UnicodeChar) {
      ...
      case '-':
        ...
        if (Tag->Operand == EFI_IFR_ORDERED_LIST_OP) {
          ...
          for (Index = MenuOption->TagIndex + TopOptionIndex; MenuOption->Tags[Index].Operand != EFI_IFR_END_ONE_OF_OP; Index++) {
