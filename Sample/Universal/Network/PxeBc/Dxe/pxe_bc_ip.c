...
--*/
{
  ...
  for (Index = 0; Index < PxeBcMode->RouteTableEntries; ++Index)  {
    if (!EfiCompareMem(&PxeBcMode->RouteTable[Index].GwAddr, RouterIpPtr,
      Private->IpLength))
    {
      return;
    }
  ...
  EFI_IP_ADDRESS      *DestIpPtr)
{
  ...
  for (Index = 0; Index < PxeBcMode->RouteTableEntries; ++Index) {
    if (OnSameSubnet(Private->IpLength, &PxeBcMode->RouteTable[Index].IpAddr,
      DestIpPtr, &PxeBcMode->RouteTable[Index].SubnetMask))
    {
