...
// length macros
#define IP_ADDRESS_LENGTH(qp) \
  (((qp)->UsingIpv6) ? sizeof(EFI_IPv6_ADDRESS) : sizeof(EFI_IPv4_ADDRESS))
...
#define IPV4_HEADER_LENGTH(IpHeaderPtr) \
  (((IpHeaderPtr)->VersionIhl & 0xf) << 2)
...
typedef struct {
  ...
  UINT8         GenerallyUnused[4];   // generally unused except byte [0] for
                                      // parameter problem message
  IPV4_HEADER   IpHeader;             // original message ip header of plus 64
                                      // bits of data
...
typedef struct {
  ...
  UINT16 Checksum;        // ones complement of ones complement sum of 16 bit
...
typedef struct {
  ...
  UINT16 HlenResCode;     // Nibble of HLEN (length of header in 32-bit multiples)
                          // 6bits of RESERVED
                          // Nibble of Code Bits 
  UINT16 Window;          // Software buffer size (sliding window size) in network-standard byte order
  UINT16 Checksum;        // ones complement of ones complement sum of 16 bit words of 
...
//
EFI_STATUS
IpFilter(PXE_BASECODE_DEVICE *Private, IN EFI_PXE_BASE_CODE_IP_FILTER *Filter);
...
UINTN
AtoU(UINT8 *BufferPtr);

UINT64
AtoU64(UINT8 *BufferPtr);
...
VOID
IgmpCheckTimers(PXE_BASECODE_DEVICE *Private);   // poll when doing a receive
...
#define Ip4AddRouter(Private, Ipv4Ptr) \
  IpAddRouter(Private, (EFI_IP_ADDRESS *)Ipv4Ptr)
...
Ip4Send(
  ...
  IN UINTN                MsgLength);   // data byte length
