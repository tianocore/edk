...
GetOffers(PXE_BASECODE_DEVICE *Private)
{
  ...
  for (Index = 0;
    Index < (sizeof Private->ServerCount) / sizeof Private->ServerCount[0];
    ++Index)
  {
  ...
  BOOLEAN             SortOffers)
{
    ...
    Private->Timeout <<= 1)
  {
    ...
    for (Index = 0; Index < NumOffers; ++Index) {
      ...
      else if (!(RxBuf[Index].OpAdds.Status &
        (DISCOVER_TYPE | WfM11a_TYPE | PXE_TYPE)))
      {
        // it is - just check ARP
        if (!RxBuf[Index].OpAdds.PktOptAdds[OP_DHCP_BOOTFILE_IX-1] ||
          !TryFinishDORA(Private, Index)) 
        {
          continue;
        }
      } else if (TryFinishDORA(Private, Index)) {
        if (!(RxBuf[Index].OpAdds.Status & (DISCOVER_TYPE | WfM11a_TYPE)) &&
          !TryFinishBINL(Private, Index)) 
        {
          continue;
        }
      }
...
)
{
  ...
  for (Index = 0; Index < ServerListPtr->Ipv4List.IpCount; ++Index) {
    if (!EfiCompareMem(ServerIpPtr, &ServerListPtr->Ipv4List.IpList[Index],
      sizeof(EFI_IPv4_ADDRESS)))
    {
      return TRUE;
    }
...
--*/
{
  ...
  // if we use ucast discovery or must use list, there better be one
    ...
    if (ServerListPtr == NULL) {
      ...
      for (Index = Index2 = 0; Index < DiscoverInfoPtr->IpCnt; ++Index) {
        if (DiscoverInfoPtr->SrvList[Index].Type == Type) {
          EfiCopyMem(&ServerListPtr->Ipv4List.IpList[Index2++],
