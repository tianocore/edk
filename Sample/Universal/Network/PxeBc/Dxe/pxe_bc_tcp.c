...
--*/
{
  ...
  if (BufferSizePtr == NULL || BufferPtr == NULL || DestIpPtr == NULL ||
    DestPortPtr == NULL || HeaderPtr == NULL ||
...
#define Tcpv4Base ((TCPV4_HEADERS *)(IpTxBuffer->u.Data - sizeof(TCPV4_PSEUDO_HEADER)))
  ...
  Tcpv4Base->Tcpv4Header.Checksum = IpChecksum2((UINT16 *)Tcpv4Base,
...
--*/
{
  ...
  // Issue BC command
  //
  StatCode = TcpWrite(Private, OpFlags, UrgentPointer, SequenceNumber,
    AckNumber, HlenResCode, Window, DestIpPtr, DestPortPtr, GatewayIpPtr, 
    SrcIpPtr, SrcPortPtr, HeaderSizePtr, HeaderPtr, BufferSizePtr, BufferPtr);
...
#pragma pack()
  ...
  for (;;) {
    *BufferSizePtr = BufferSize;
    ...
    //
    StatCode = IpReceive(Private, OpFlags, SrcIpPtr, DestIpPtr, PROT_TCP,
      HeaderPtr, HeaderSize, BufferPtr, BufferSizePtr, 0);

    EfiCopyMem (&Hdrs.Ipv4Hdr, HeaderPtr, HeaderSize);

    DEBUG((EFI_D_NET, "\nTcpRead()  BufferSize = %xh  Ipv+Tcp = %d",
      *BufferSizePtr, sizeof Hdrs.Ipv4Hdr + sizeof Hdrs.Tcpv4Hdr));

    DEBUG((EFI_D_NET, "\nTcpRead()  Destination IP address is:  %d.%d.%d.%d\n",
      Hdrs.Ipv4Hdr.DestAddr.B[0], Hdrs.Ipv4Hdr.DestAddr.B[1],
      Hdrs.Ipv4Hdr.DestAddr.B[2], Hdrs.Ipv4Hdr.DestAddr.B[3]));

    DEBUG((EFI_D_NET, "\nTcpRead()  Source IP address is:  %d.%d.%d.%d\n",
      Hdrs.Ipv4Hdr.SrcAddr.B[0], Hdrs.Ipv4Hdr.SrcAddr.B[1],
      Hdrs.Ipv4Hdr.SrcAddr.B[2], Hdrs.Ipv4Hdr.SrcAddr.B[3]));
    ...
    if (StatCode == EFI_SUCCESS || StatCode == EFI_BUFFER_TOO_SMALL) {
      ...
      if (StatCode == EFI_SUCCESS && Hdrs.Tcpv4Hdr.Checksum) {
        ...
        Hdrs.Tcpv4Phdr.TotalLength = (UINT16)(NTOHS(Hdrs.Ipv4Hdr.TotalLength) -
          sizeof Hdrs.Ipv4Hdr);
        ...
        //
        if (IpChecksum2((UINT16 *)HeaderPtr + sizeof Hdrs.Ipv4Hdr,
...
--*/
{
  ...
  StatCode = TcpRead(Private, OpFlags, DestIp, DestPort, SrcIp, SrcPort,
