...
Abstract:
  ...
  The driver for Deferred Procedure Call (DPC) Protocol.
...
EFI_LIST_ENTRY  mDpcEntryFreeList = INITIALIZE_LIST_HEAD_VARIABLE(mDpcEntryFreeList);
...
//
EFI_LIST_ENTRY  mDpcQueue[EFI_TPL_HIGH_LEVEL + 1];
...
--*/
{
  ...
  //
  if (DpcTpl < EFI_TPL_APPLICATION || DpcTpl > EFI_TPL_HIGH_LEVEL) {
  ...
  if (IsListEmpty (&mDpcEntryFreeList)) {
    ...
    for (Index = 0; Index < 64; Index++) {
      ...
      //
      gBS->AllocatePool(EfiBootServicesData, sizeof(DPC_ENTRY), &DpcEntry);

      //
      // Raise the TPL level back to TPL_HIGH_LEVEL for DPC list operations
      //
      gBS->RaiseTPL (EFI_TPL_HIGH_LEVEL);
  ...
  
{
  ...
  if (mDpcQueueDepth > 0) {
    ...
    //
    for (Tpl = EFI_TPL_HIGH_LEVEL; Tpl >= OriginalTpl; Tpl--) {
      //
...
EFI_DRIVER_ENTRY_POINT (DpcDriverEntryPoint)
