...
--*/
{
  ...
  while (!EfiCompareMem(&rx_pkt->dhcp4.magik, &magik, 4)) {
    ...
    EfiStatus = find_opt(rx_pkt, DHCP4_SERVER_IDENTIFIER,
      0, &srvid_op);
...
--*/
{
  ...
  if (Private->RebindTime < 52 || Private->RebindTime >= Private->LeaseTime) {
    Private->RebindTime = Private->LeaseTime / 2 + Private->LeaseTime / 4 +
      Private->LeaseTime / 8;
  }
  
  if (Private->RenewTime < 45 || Private->RenewTime >= Private->RebindTime) {
    Private->RenewTime = Private->RebindTime / 2 + Private->RebindTime / 4 +
      Private->RebindTime / 8;
  }
  ...
  OUT DHCP4_PACKET **OfferList)
{
  ...
  *OfferList = NULL;
  ...
  EfiStatus = tx_rx_udp(Private, &bcast_ip, NULL, NULL, NULL,
