  ...
  )
{
  ...
  SecretSize  = (UINT32) EfiAsciiStrLen (AuthData->AuthConfig.ReverseCHAPSecret);
...
--*/
{
  ...
  case ISCSI_CHAP_INITIAL:
    ...
    Session->TargetPortalGroupTag = (UINT16) NetAtoi (Value);
    ...
    //
    if (EfiAsciiStrCmp (Value, ISCSI_AUTH_METHOD_CHAP) == 0) {
  ...
  case ISCSI_CHAP_STEP_TWO:
    ...
    //
    AuthData->InIdentifier      = (UINT32) NetAtoi (Identifier);
    AuthData->InChallengeLength = ISCSI_CHAP_AUTH_MAX_LEN;
    IScsiHexToBin ((UINT8 *) AuthData->InChallenge, &AuthData->InChallengeLength, Challenge);
    Status = IScsiCHAPCalculateResponse (
              AuthData->InIdentifier,
              AuthData->AuthConfig.CHAPSecret,
              (UINT32) EfiAsciiStrLen (AuthData->AuthConfig.CHAPSecret),
...
--*/
{
  ...
  case ISCSI_CHAP_STEP_ONE:
    ...
    //
    ASPrint (ValueStr, sizeof (ValueStr), "%d", ISCSI_CHAP_ALGORITHM_MD5);
  ...
  case ISCSI_CHAP_STEP_THREE:
    ...
    if (AuthData->AuthConfig.CHAPType == ISCSI_CHAP_MUTUAL) {
      ...
      ASPrint (ValueStr, sizeof (ValueStr), "%d", AuthData->OutIdentifier);
