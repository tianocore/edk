...
--*/
{
  ...
  for (Index = 0; (Index < 4) && (LunUnitStr[Index] != NULL); Index++) {
    if (EfiAsciiStrLen (LunUnitStr[Index]) > 4) {
      return EFI_INVALID_PARAMETER;
    }

    *((UINT16 *) &Lun[Index * 2]) = HTONS (NetXtoi (LunUnitStr[Index]));
  }
...
--*/
{
  ...
  for (Index = 0; Index < 4; Index++) {
    ...
    if ((Lun[2 * Index] | Lun[2 * Index + 1]) == 0) {
      ...
      EfiStrTrim (TempStr, L'0');
    }

    TempStr += EfiStrLen (TempStr);
  }

  Str[EfiStrLen (Str) - 1] = 0;

  for (Index = EfiStrLen (Str) - 1; Index > 1; Index = Index - 2) {
...
--*/
{
  ...
  Private->IScsiExtScsiPassThru = gIScsiExtScsiPassThruProtocolTemplate;
...
--*/
{
  ...
  // Duplicate it.
  //
  DevicePath  = EfiDuplicateDevicePath (DevicePath);
