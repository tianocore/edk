...
--*/
{
  ...
  (*AdapterInfo->Mem_Io) (AdapterInfo->Unique_ID, 
                          PXE_MEM_READ, 
                          1, 
                          (UINT64)Port,
                         (UINT64)&Results);
...
{
  UINT64 *PhyAddr = (UINT64 *)(UINTN)MappedAddr;
  ...
  if (AdapterInfo->VersionFlag == 0x30) {
    ...
    if (AdapterInfo->Map_Mem == (VOID *)NULL) { 
      ...
      (*AdapterInfo->Map_Mem) (AdapterInfo->Unique_ID,
                               MemAddr,
                               Size,
                               Direction,
                               MappedAddr);
...
{
  if (AdapterInfo->VersionFlag > 0x30) {
    if (AdapterInfo->UnMap_Mem != (VOID *)NULL) { // no mapping service
      (*AdapterInfo->UnMap_Mem) (AdapterInfo->Unique_ID,
                                 MemAddr,
                                 Size,
                                 Direction,
                                 MappedAddr);
...
UINT8
Load_Base_Regs (NIC_DATA_INSTANCE *AdapterInfo)
{
  //
...
UINT8
IssueCB (NIC_DATA_INSTANCE *AdapterInfo, TxCB *cmd_ptr)
{
  UINT16 status;
  ...
  //
  if ((status & SCB_STATUS_CNA) != 0)  // ack only the cna the integer
  ...
  } else {  // either active or suspended, give a resume
...
UINT8
Configure (NIC_DATA_INSTANCE *AdapterInfo)
{
  TxCB    *cmd_ptr;   // all command blocks are of TxCB format 
  UINT8   *data_ptr;
  INT16   Index;
  UINT8   my_filter;
...
UINT8
E100bSetupIAAddr (NIC_DATA_INSTANCE *AdapterInfo)
{
  TxCB *cmd_ptr;  // all command blocks are of TxCB format 
  UINT16 *data_ptr;
  UINT16 *eaddrs = (UINT16 *)AdapterInfo->CurrentNodeAddress;
...
INT8
StartRU (NIC_DATA_INSTANCE *AdapterInfo)
/*++
...
--*/
{
  ...
  AdapterInfo->PhyAddress = 0xFF;  // auto detect.
...
--*/
{
  ...
  }  else { // disable ints, should not be given for SW Int.
    OutWord (AdapterInfo, INT_MASK, AdapterInfo->ioaddr + SCBCmd);
  }
...
UINTN
E100bSetfilter (NIC_DATA_INSTANCE *AdapterInfo, UINT16 new_filter,
...
--*/
{
  ...
  if ( (old_filter & cfg_flt) != (new_filter & cfg_flt)) {
    ...
    Configure (AdapterInfo); // do we have to reset the chip ?
  }
  ...
  if ( ((new_filter & PXE_OPFLAGS_RECEIVE_FILTER_FILTERED_MULTICAST) !=
    ...
    XmitWaitForCompletion (AdapterInfo);  // before issuing any new command!
...
UINTN
E100bTransmit (NIC_DATA_INSTANCE *AdapterInfo, UINT64 cpb, UINT16 opflags)
{
...
UINTN
E100bReceive (NIC_DATA_INSTANCE *AdapterInfo, UINT64 cpb, UINT64 db)
{
  ...
  status = AdapterInfo->Int_Status; // include the prev ints as well
  ...
  while ( (status = rx_ptr->cb_header.status) & RX_COMPLETE) {
    ...
    // discard bad frames
    //
    if ((status & 0x0D90) != 0)  // crc, align, dma overrun, too short, receive error (v22 no coll)
      goto FreeRFD;
    ...
    if (pkt_len != 0) {
      ...
      //
      rx_dbptr->FrameLen = pkt_len;  // includes header
      ...
      if (Index >= PXE_HWADDR_LEN_ETHER) {
        ...
        for (Index=0; Index<PXE_HWADDR_LEN_ETHER; Index++) {
          ...
          pkt_type = PXE_FRAME_TYPE_BROADCAST;
                ...
                if ( (hdr_ptr->dest_addr[0] & 1) == 1)  {// mcast
...
INT16
E100bReadEepromAndStationAddress (NIC_DATA_INSTANCE *AdapterInfo)
{
  INT32   Index;
  INT32   Index2;
  UINT16  sum;
  UINT16  eeprom_len;
  UINT8   addr_len;
  UINT16  *eedata = (UINT16 *) (&AdapterInfo->NVData[0]);

  sum = 0;
  addr_len = E100bGetEepromAddrLen (AdapterInfo);
  AdapterInfo->NVData_Len = eeprom_len = (UINT16) (1 << addr_len);  // in words

  for (Index2 = 0, Index = 0; Index < eeprom_len; Index++) {
    UINT16 value = E100bReadEeprom (AdapterInfo, Index, addr_len);
    eedata[Index] = value;
...
//
UINT8
SetupCBlink (NIC_DATA_INSTANCE *AdapterInfo)
{
  TxCB  *head_ptr;
  TxCB  *tail_ptr;
  TxCB  *cur_ptr;
  ...
  for (Index=0; Index < AdapterInfo->TxBufCnt; Index++)  {
    ...
    cur_ptr[Index].PhysTCBAddress = (UINT32)AdapterInfo->tx_phy_addr + (Index * sizeof (TxCB));
    cur_ptr[Index].PhysTBDArrayAddres = cur_ptr[Index].PhysArrayAddr =
...
TxCB *
GetFreeCB (NIC_DATA_INSTANCE *AdapterInfo)
{
  TxCB *free_cb_ptr;
  ...
  )
{
  ...
  for (Index=0; Index < AdapterInfo->RxBufCnt; Index++) {
    ...
    rx_ptr[Index].rx_buf_addr = (UINT32) (-1);  // RBDs not used, simple memory model

    rx_ptr[Index].forwarded = FALSE;  // RBDs not used, simple memory model
  ...
  tail_ptr->cb_header.command = 0xC000;  // set the EL bit
  ...
  )
{
  ...
  rx_ptr->cb_header.command = 0xc000;  //set el_bit and suspend bit
  rx_ptr->cb_header.status = 0;
  rx_ptr-> ActualCount = 0;
  rx_ptr->forwarded = FALSE;
  AdapterInfo->RFDTailPtr = rx_ptr;
  tail_ptr->cb_header.command = 0; // resetting the el_bit.
  ...
  )
{
  INT32 Index;
  UINT8 Tmp;
  UINT32 EEAddr = AdapterInfo->ioaddr + SCBeeprom;

  for (Index = num_bits; Index >= 0; Index--) {
    INT16 dataval = (INT16) ( (val & (1 << Index)) ? EE_DI : 0); // will be 0 or 4
    Tmp = (UINT8) (InByte (AdapterInfo, EEAddr) & ~EE_DI); // mask off the data_in bit
    Tmp |= dataval;
  ...
  )
{
  ...
  for (Index = 15; Index >= 0; Index--) {
    ...
    // raise the clock
    //
    Tmp = InByte (AdapterInfo, EEAddr); // mask off the data_in bit
    OutByte (AdapterInfo, (UINT8) (Tmp | EE_SHIFT_CLK), EEAddr);
    eeprom_delay (100);
...
--*/
{
  ...
  UINT32 EEAddr = AdapterInfo->ioaddr + SCBeeprom;
  UINT16 ReadCmd = (UINT16) (Location | (EE_READ_CMD << AddrLen));
  ...
  //
  Tmp = (UINT8) (InByte (AdapterInfo, EEAddr) & 0xF2);  // 11110010b
  OutByte (AdapterInfo, (UINT8) (Tmp | EE_CS), EEAddr);

  shift_bits_out (AdapterInfo, ReadCmd, (UINT8) (3+AddrLen));  // 3 for the read opcode 110b
...
)
{
  ...
  UINT32 Wait;  // wait upto one second (each wait is 100 micro s)
  ...
  db.Data[0x04] = db.Data[0x02] + db.Data[0x08] +
  ...
  db.Data[0x0A] = db.Data[0x0B] + db.Data[0x0E] +
  ...
  IN INT32             OpFlags)
{
  ...
  if ((OpFlags & PXE_OPFLAGS_RESET_DISABLE_INTERRUPTS) != 0) {
    AdapterInfo->int_mask =0;  // disable the interrupts
  }
  //
...
--*/
{
  ...
  WriteCommand = ((UINT32) DataValue) | ((UINT32)(RegAddress << 16)) | 
                 ((UINT32)(PhyAddress << 21)) | ((UINT32)(MDI_WRITE << 26));
...
--*/
{
  UINT32 ReadCommand = ((UINT32)(RegAddress << 16)) | 
...
VOID
XmitWaitForCompletion (NIC_DATA_INSTANCE *AdapterInfo)
{
  TxCB *TxPtr;
...
INT8
CommandWaitForCompletion (TxCB *cmd_ptr, NIC_DATA_INSTANCE *AdapterInfo)
{
  INT16 wait; // total wait time is 50 milli seconds??
...
INT8
SoftwareReset (NIC_DATA_INSTANCE *AdapterInfo)
{
  UINT8 tco_stat;
  UINT16 wait;
  ...
  DelayIt (AdapterInfo, 5000); // wait for 5 milli seconds here!
  ...
  if (AdapterInfo->VendorID == D102_DEVICE_ID ||
    ...
    //
    wait = 5000;  // wait for 5 milli seconds
    while (wait) {
      tco_stat = InByte (AdapterInfo, AdapterInfo->ioaddr + 0x1b);
      if ( (tco_stat & 2) == 0) {  // is the activity bit clear??
        break;
      }
      wait--;
      DelayIt (AdapterInfo, 1);
    }
    if ((tco_stat & 2) != 0)  {// not zero??
      return -1;
    }
  }
  return 0;
  ...
  )
{
  ...
  //
  DelayIt (AdapterInfo, 2000); // wait for 2 milli seconds here!
  while (wait > 0) {
    wait--;
    ...
    DelayIt (AdapterInfo, 1000);  // wait for 1 milli second
  }
  if (stat != 0) {
    return PXE_STATCODE_DEVICE_FAILURE;
  }
