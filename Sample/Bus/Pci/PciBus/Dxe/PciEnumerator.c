...
--*/
{
  ...
  //ResetBus (PciRootBridgeIo, StartBusNumber);
  *SubBusNumber = StartBusNumber;
  ...
  *pConfig = NULL;
  ...
  if (NumConfig != 0) {
    ...
    if (Aperture & 0x01) {
      ...
      Ptr->SpecificFlag = 1;                                      // non ISA range
    ...
    if (Aperture & 0x02) {
      ...
      Ptr->SpecificFlag         = 0;                              // Nonprefechable
    ...
    if (Aperture & 0x10) {
      ...
      Ptr->Len                  = sizeof (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR) - 3;
      Ptr->ResType              = ACPI_ADDRESS_SPACE_TYPE_MEM;            // Mem 
      Ptr->SpecificFlag         = 0x06;                                   // prefechable
      Ptr->AddrSpaceGranularity = 64;                                     // 32 bit
