...
--*/  
{
  ...
  case 0x00: // Magnetic Disk
    ...
    IdeDev->BlkIo.Media->MediaPresent = FALSE;    // Give initial value
...
--*/  
{
  ...
  IDEWritePortB (IdeDev->PciIo,IdeDev->IoPort->Head, 
                (UINT8)((IdeDev->Device << 4) | DEFAULT_CMD)
...
{                                                      
  ...
  while (ActualWordCount < RequiredWordCount) { 
    ...
    WordCount = ((IDEReadPortB (IdeDev->PciIo,IdeDev->IoPort->CylinderMsb) << 8) | 
                  IDEReadPortB (IdeDev->PciIo,IdeDev->IoPort->CylinderLsb)) & 0xffff;                  
    WordCount /= 2;                                         
...
--*/  
{
  ...
  *SenseCounts = 0 ;
  ...
  Ptr = (UINT16*)IdeDev->SenseData ; // initialize pointer
  AllocateTimes = 0;
  ...
  for (FetchSenseData = TRUE; FetchSenseData;) {
    ...
    //
    Status = AtapiPacketCommandIn (IdeDev, 
                                    &Packet, 
                                    Ptr, 
    ...
    if ((Sense->sense_key != SK_NO_SENSE) && ((*SenseCounts) < 20)) {      
      ...
      Ptr += sizeof(REQUEST_SENSE_DATA) / 2 ; // Ptr is word-based pointer      
      ...
      if (*SenseCounts == IdeDev->SenseDataNumber ) {
        ...
        AllocateTimes ++ ;
        AllocateSize = sizeof(REQUEST_SENSE_DATA) * AllocateUnit 
                        * (AllocateTimes + 1) ;
...
--*/  
{
  ...
  *MediaChange = FALSE;
  ...
  for (RetryIndex = 0; (RetryIndex < RetryTimes) 
        && (RetryIndex < MaximumRetryTimes); RetryIndex ++) {  
...
--*/  
{
  ...
  while (BlocksRemaining > 0) {
    ...
    Status = AtapiPacketCommandIn(
              IdeDev, &Packet, (UINT16 *)PtrBuffer,
              ByteCount,TimeOut
              );
    if (EFI_ERROR(Status)) {
      return Status ;
