...
Abstract: 
  ...
  The UHCI memory operation routines.
...
--*/
{
  ...
  for (TempHeaderPtr = HcDev->MemoryBank->Next; TempHeaderPtr != NULL;) {

    DelinkMemoryBlock (HcDev->MemoryBank, TempHeaderPtr);
    //
    // when the TempHeaderPtr is freed in FreeMemoryHeader(),
    // the TempHeaderPtr is pointing to nonsense content.
    //
    ...
    //
    TempHeaderPtr = HcDev->MemoryBank->Next;
  }

  FreeMemoryHeader (HcDev, HcDev->MemoryBank);
