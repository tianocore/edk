  ...
  )
{
  ...
  switch(Index) {    
  case 1: // Primary Master
    Fmt = L"Primary Master %s";
    break;
    
  case 2: // Primary Slave
    Fmt = L"Primary Slave %s";
    break;
    
  case 3: // Secondary Master
    Fmt = L"Secondary Master %s";
    break;
    
  case 4: // Secondary Slave
  ...
  )
{
  ...
  *((UINT16*)Ptr) = (UINT16)Index;
  ...
  Status = gRT->SetVariable (BootString,
  ...
  NewBootOrderList = EfiAllocateZeroPool (
  ...
  )
{
  UINT16                         *BootOrder;
  UINT8                          *BootOptionVar;
  UINTN                          BootOrderSize, BootOptionSize;
  EFI_STATUS                     Status;
  UINT16                         HddCount;
  UINT16                         BbsCount;
  HDD_INFO                       *LocalHddInfo;
  BBS_TABLE                      *LocalBbsTable, *BbsEntry;
  UINT16                         BbsIndex;
  ...
  )
{
  ...
  if (NULL == DevPath) return EFI_OUT_OF_RESOURCES;
  ...
  )
{
  ...
  for (Index = 0; Index < BbsCount; Index++) {
    if (BbsTable[Index].BootPriority == BBS_IGNORE_ENTRY) continue;
    if (BbsTable[Index].DeviceType != BbsType) continue;
  ...
  )
{
  ...
  if (NULL == DevOrder) return EFI_OUT_OF_RESOURCES;
  Ptr = DevOrder;
  ...
  *((UINT16*)Ptr) = (UINT16)(sizeof (UINT16) + FDCount * sizeof (UINT16));
  Ptr += sizeof (UINT16);
  if (FDCount) Ptr = (UINT8*)BdsFillDevOrderBuf (BbsTable, BBS_FLOPPY, BbsCount, (UINT16*)Ptr);
 
  *((BBS_TYPE*)Ptr) = BBS_HARDDISK;
  Ptr += sizeof (BBS_TYPE);
  *((UINT16*)Ptr) = (UINT16)(sizeof (UINT16) + HDCount * sizeof (UINT16));
  Ptr += sizeof (UINT16);
  if (HDCount) Ptr = (UINT8*)BdsFillDevOrderBuf (BbsTable, BBS_HARDDISK, BbsCount, (UINT16*)Ptr);
    
  *((BBS_TYPE*)Ptr) = BBS_CDROM;
  Ptr += sizeof (BBS_TYPE);
  *((UINT16*)Ptr) = (UINT16)(sizeof (UINT16) + CDCount * sizeof (UINT16));
  Ptr += sizeof (UINT16);
  if (CDCount) Ptr = (UINT8*)BdsFillDevOrderBuf (BbsTable, BBS_CDROM, BbsCount, (UINT16*)Ptr);
    
  *((BBS_TYPE*)Ptr) = BBS_EMBED_NETWORK;
  Ptr += sizeof (BBS_TYPE);
  *((UINT16*)Ptr) = (UINT16)(sizeof (UINT16) + NETCount * sizeof (UINT16));
  Ptr += sizeof (UINT16);
  if (NETCount) Ptr = (UINT8*)BdsFillDevOrderBuf (BbsTable, BBS_EMBED_NETWORK, BbsCount, (UINT16*)Ptr);

  *((BBS_TYPE*)Ptr) = BBS_BEV_DEVICE;
  Ptr += sizeof (BBS_TYPE);
  *((UINT16*)Ptr) = (UINT16)(sizeof (UINT16) + BEVCount * sizeof (UINT16));
  Ptr += sizeof (UINT16);
  if (BEVCount) Ptr = (UINT8*)BdsFillDevOrderBuf (BbsTable, BBS_BEV_DEVICE, BbsCount, (UINT16*)Ptr);

  Status = gRT->SetVariable (VarLegacyDevOrder,
...
--*/
 {
   ...
   if (NULL == DevOrder) return BdsCreateDevOrder(LocalBbsTable, BbsCount);
   //
   ...
   *((UINT16*)NewPtr) = (UINT16)(sizeof(UINT16) + BEVCount * sizeof (UINT16));
   ...
   for (Index = 0; Index < BbsCount; Index++) {
     if ((LocalBbsTable[Index].BootPriority == BBS_IGNORE_ENTRY) ||
        (LocalBbsTable[Index].BootPriority == BBS_DO_NOT_BOOT_FROM) ||
        (LocalBbsTable[Index].BootPriority == BBS_LOWEST_PRIORITY)) continue;
     switch (LocalBbsTable[Index].DeviceType) {
     case BBS_FLOPPY:
       Idx = &FDIndex;
       NewDevPtr = NewFDPtr;
     ...
     if (Idx) {
       for (Index2 = 0; Index2 < *Idx; Index2++) {
         if ((NewDevPtr[Index2] & 0xFF) == (UINT16)Index) break;
       }
       if (Index2 == *Idx) {
         //
  ...
  )
{
  ...
  for (Idx =0 ; Idx < MAX_BBS_ENTRIES; Idx++) {
    ...
    DEBUG ((EFI_D_ERROR, " %02x: %04x %02x/%02x/%02x %02x/02%x %04x %04x %04x:%04x\n",
      (UINTN)Idx,
  ...
  )
{
  ...
  for (Index = 0; BootOrder && Index < BootOrderSize / sizeof (UINT16); Index++) {
    ...
    if (BBS_DEVICE_PATH != DevPath->Type ||
        BBS_BBS_DP != DevPath->SubType) {
      SafeFreePool (BootOptionVar);
      continue;
    }
