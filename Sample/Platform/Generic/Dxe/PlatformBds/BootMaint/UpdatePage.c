  ...
  )
{
  ...
  CreateStringOpCode (DRIVER_ADD_OPTION_QUESTION_ID,
                      (UINT8) 150,
  ...
  )
{
  ...
  if (NumberOfOptions > 0) {
    ...
    IfrOptionList = EfiAllocateZeroPool ((NumberOfOptions + 1) *
                                         sizeof (IFR_OPTION));
 ...
 )
{
  ...
  for (Index = 0; Index < OrderSize / sizeof (UINT16); Index ++)
  {
    ...
    if ((DeviceType == ((BBS_TABLE *)OptionalData)->DeviceType) &&
      (BBS_DEVICE_PATH == DevicePath->Type) && (BBS_BBS_DP == DevicePath->SubType)) {
  ...
  )
{
  ...
  } else if ((NewPageId == KEY_VALUE_SAVE_AND_EXIT) || 
             (NewPageId == KEY_VALUE_NO_SAVE_AND_EXIT) ) {
    //
