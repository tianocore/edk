...
--*/
{
  ...
  FileHeader  = NULL;
  do {
    Status = FfsFindNextFile (SearchType, FwVolHeader, &FileHeader);
    if (!EFI_ERROR (Status)) {
      if (FileName == NULL || EfiCompareGuid (FileName, &FileHeader->Name)) {
        Status = FfsFindSectionData (EFI_SECTION_PE32, FileHeader, Pe32Data, Pe32Size);
        return Status;
      }
    }
