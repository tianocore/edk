...
Abstract:
  ...
  EFI 2.0 PE/COFF loader. This version is IA32 code that loades x64 images 
...
--*/
{
  ...
  if (!(IMAGE_MACHINE_TYPE_SUPPORTED (ImageContext->Machine))) {
...
--*/
{
  ...
  Status = PeCoffLoaderGetPeHeader (ImageContext, &PeHdr);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  // Verify machine type
  //
  Status = PeCoffLoaderCheckImageType (ImageContext, &PeHdr);
  if (EFI_ERROR(Status)) {
    return Status;
  }
...
--*/
{
  ...
  for ( Index=0, MaxEnd = NULL; 
    ...
    if (Size < Section->Misc.VirtualSize) {
      EfiZeroMem (Base + (UINTN)Size, Section->Misc.VirtualSize - (UINTN)Size);
    }
  ...
  ImageContext->EntryPoint = 
      (EFI_PHYSICAL_ADDRESS) (UINTN) PeCoffLoaderImageAddress (
                                    ImageContext, 
