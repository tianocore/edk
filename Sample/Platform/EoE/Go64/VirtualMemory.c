  ...
  for (Index = 0; Index < MaxBitsSupported; Index++, PageMapLevel4Entry++) {
    ...
    for (Index1 = 0; Index1 < 512; Index1++, PageDirectoryPointerEntry++) {
      ...
      for (Index2 = 0; Index2 < 512; Index2++, PageDirectoryEntry2MB++, PageAddress += 0x200000) {
        ...
        //
        // Retrieve the MTRR memory type for the 2MB region starting at PageAddress
        //
        MtrrMemoryType = EfiGetMTRRMemoryType (PageAddress);

        switch (MtrrMemoryType) {
        case WriteCombining:
          PageDirectoryEntry2MB->Bits.CacheDisabled = 1;
          PageDirectoryEntry2MB->Bits.WriteThrough  = 1;
          break;
        case WriteThrough:
          PageDirectoryEntry2MB->Bits.WriteThrough = 1;
          break;
        case WriteProtected:
          PageDirectoryEntry2MB->Bits.ReadWrite = 0;
          break;
        case WriteBack:
          break;
        case Uncached:
