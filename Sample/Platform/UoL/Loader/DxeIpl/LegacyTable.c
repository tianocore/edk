  ...
  )
{
  ...
  if (Hob->Acpi20.Table > 0) {
    ...
    //
    if (Xsdt != NULL) {
      ScanTableInXSDT (Xsdt, MCFG_SIGNATURE, &McfgTable);
    }
    //
    // Check Rsdt
    //
    if ((McfgTable == NULL) && (Rsdt != NULL)) {
      ScanTableInRSDT (Rsdt, MCFG_SIGNATURE, &McfgTable);
    }
  }
  ...
  // Check ACPI1.0 table
  //
  if ((McfgTable == NULL) && (Hob->Acpi.Table > 0)) {
