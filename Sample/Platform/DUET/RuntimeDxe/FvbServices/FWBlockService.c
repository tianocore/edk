...
--*/
{
  ...
  Status = EFI_SUCCESS;
  EfiCopyMem ((VOID *) Address, Buffer, *NumBytes);
...
--*/
{
  ...
  EfiSetMem ((VOID *)Address, LbaLength, 0xff);
...
--*/
{
  ...
  EfiCopyMem (Buffer, (VOID *) (LbaAddress + BlockOffset), (UINTN) *NumBytes);
...
--*/
{
  ...
  do {
    ...
    if (*MappedFile != '\0') {
      ...
      EfiStrCpy (FwhInstance->MappedFile, MappedFile);
    ...
    if (FwhInstance->WriteEnabled) {
      for (PtrBlockMapEntry = FwVolHeader->FvBlockMap; PtrBlockMapEntry->NumBlocks != 0; PtrBlockMapEntry++) {
        ...
        LbaAddress += PtrBlockMapEntry->NumBlocks * PtrBlockMapEntry->BlockLength;
        //
